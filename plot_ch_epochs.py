...
Y_AXIS = {"min": -0.00010, "max": 0.00010, "step": 0.00005}
...
def axes_formatting(ax):
    ...
    labels = map(lambda y: "{:.0f}".format(y * 1000000), y_majors)
...
def plot_channels_epochs(epoch_list, Fs, output_path):
    n = len(epoch_list)
    ...
    for ch_i, ch in enumerate(_CHANNELS):
        ...
        plt.savefig(output_path.replace("ica", ch), dpi=C_DPI)
        plt.cla()
        plt.close()
